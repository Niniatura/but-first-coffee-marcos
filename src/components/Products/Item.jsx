...
function Item (props) {
    let { title, price, img, detail, categoria } = props;
    return (
        <div className="my-10 tablet:w-64 mobile:w-80 h-auto">
            <div className="w-full max-w-sm bg-white rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700 flex flex-col items-center">
                <a href="#">
                    <img className="m-2 w-48 rounded-lg object-scale-down" src={img} alt="product image" />
                </a>
                <div className="px-5 pb-5">
                        <h5 className="text-sm font-semibold tracking-tight text-gray-900 dark:text-white text-center h-10">{title}</h5>
                    <div className="flex items-center mt-2.5">
                        <p>{detail.length > 100 ? `${detail.substring(0, 60)}...` : detail}</p>
                    </div>
